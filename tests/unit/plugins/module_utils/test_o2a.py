...
testcases = [
    ...
    ({}, []),
    ("quiet", ["--quiet"]),
    (["quiet", "config-cache"], ["--quiet", "--config-cache"]),
    ({"prefix": "/usr/local/openafs"}, ["--prefix=/usr/local/openafs"]),
    (
    ...
    ({"enable": ["debug", "debug-lwp"]}, ["--enable-debug", "--enable-debug-lwp"]),
    (
        {"enable": ["debug", "debug-lwp"], "disable": ["kernel-module"]},
        ["--enable-debug", "--enable-debug-lwp", "--disable-kernel-module"],
    ),
    ({"with": {"krb5": "/path/to/krb5"}}, ["--with-krb5=/path/to/krb5"]),
    (
...
@pytest.mark.parametrize(
    "options", [{"bogus": {"x": "a"}}, {"enable": {"enable": {"bar": "baz"}}}]
)
